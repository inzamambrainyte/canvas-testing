...
const TextEditorPanel = () => {
  const { scenes, activeSceneId, selectedElementId, updateElement } = useEditorStore((state) => ({
    scenes: state.scenes,
  ...
  const element = activeScene?.elements.find((el) => el.id === selectedElementId);
      ...
      <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">
        ...
        <textarea
          ...
          onChange={(event) => {
            ...
            updateElement(activeSceneId, element.id, { content: event.target.value });
          }}
        />
      </label>
      <div className="grid grid-cols-2 gap-3 text-sm text-slate-600">
        <label className="space-y-1">
          ...
          <input
            ...
            onChange={(event) =>
              updateElement(activeSceneId, element.id, { fontSize: Number(event.target.value) })
            }
          />
        </label>
        <label className="space-y-1">
          ...
          <select
            ...
            onChange={(event) =>
              updateElement(activeSceneId, element.id, { fontFamily: event.target.value })
            }
          >
